...
import { industries, chooseIndustry, useAssessment, recommendation, resetAssessment, type IndustryId, type Pain, type WebsiteState } from "@/lib/assessment";
...
export default function AssessmentFlow({ defaultBusinessType = "" }: { defaultBusinessType?: string }) {
  ...
  const summary = `Assessment — ${industry?.name ?? "Other business"}; biggest challenge: ${profile.pain ? painLabels[profile.pain] : "Not selected"}; website today: ${profile.websiteState || "Not selected"}. Suggested starting point: ${result.label}. Scenario the visitor chose: ${profile.extraCustomers} more customers/month × ${fmt(profile.value)} per customer ≈ ${fmt(opportunity)}/month in potential new revenue. Illustrative scenario only — not a guarantee, and it excludes costs.`;
  const next = () => {
  ...
  return (
    <div className="assessment-flow">
      ...
      <h3 ref={title} tabIndex={-1} className="mb-5 font-display text-xl font-semibold outline-none">{["Let’s make this fit your business.", "Put a rough number on it.", "Your recommended starting point."][step]}</h3>
